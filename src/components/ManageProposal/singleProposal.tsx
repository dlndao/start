...
import InviteUsers from "../InviteInvestors";
...
function SingleProposal({
  ...
  isShowOnly = false, //true if user will show proposal only link invest or mfi proposals
  isApprove = false,
  isBorrow = false,
  isMfi = false,
  isIgnored = false,
  ...
  const [
    ...
    setShowAssignProposalToCampaign,
  ] = useState(false);
  const [
    approveRemoveProposalFromCampaign,
    setApproveRemoveProposalFromCampaign,
  ] = useState(false);
        ...
        inputs: {
          ...
          userName:
            !isBorrow && data.user
              ...
              : "",
          proposalCampaigns: data.proposalCampaigns
            ? data.proposalCampaigns
            : [],
        },
      });
          ...
          <Form className='app-form py-4'>
            <Row>
              <Col className='col-auto app-user-profile-img-main text-left'>
                  ...
                  <div>
                    ...
                    >
                      <div className='app-action-icon'>
                        <img
                          src={
                            data.proposalCampaigns?.length != 0 || isCampaign
                              ? assigned
                              : assign
                          }
                          alt='icon'
                          ...
                          onClick={() =>
                            (data.proposalCampaigns &&
                              data.proposalCampaigns.length != 0) ||
                            isCampaign
                              ? setApproveRemoveProposalFromCampaign(true)
                              : setShowAssignProposalToCampaign(true)
                          }
                        />
                      </div>
                    </MaterialToolTip>
                ...
                {approveRemoveProposalFromCampaign && (
                  <ConfirmationModal
                    message={`${strings.confirmDeleteProposal} ${
                      isCampaign ? campaignName : data.proposalCampaigns[0].name
                    } ?`}
                            ...
                            title={
                              isBorrow && data.status === ProposalStatus.repaid
                                ...
                                ? `Please support my ${state.inputs.proposalName} proposal on @delndao` +
                                  ` ${
                                    isCampaign
                                      ? `.... Please support the ${campaignName} campaign on DLN`
                                      : "."
                                  }`
                                : data.isBackedByUser
